...
// todo: eigenes Package ausdenken
//package org.ikt.spark.graphx.examples
...
object VneApp {
  ...
  def vertexMappingRandom(sgraph: Graph[(String,Int), (Int, Int)], vgraph: Graph[(String,Int), Int]): ListMap[Long, Long] = {
	ListMap(1L -> 1L, 2L -> 3L, 3L -> 5L)
//   to do: Node Mapping
  }

  def main(args: Array[String]): Unit =	{
    val conf = new SparkConf().setAppName("VNE").setMaster("local")
	...
	// Example 1: Zufälliges Node-Mapping, in VneApp definiert
	// Mapping
////	val nodeMapping: ListMap[Long, Long] = ListMap(1L -> 1L, 2L -> 3L, 3L -> 5L)
