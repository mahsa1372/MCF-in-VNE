...
abstract class vertexMapping {

	def vertexMappingGreedy(sArray: Array[(Long, (String, Int))], 
				vArray: Array[(Long, (String, Int))]) :Map[Int, Int]


	def updateCapacity(sArray: Array[(Long, (String, Int))], 
			   vArray: Array[(Long, (String, Int))], 
			   nmapping: Map[Int, Int]) :Unit
			   ...
			   nmapping: Map[Int, Int]) :Unit = {
                ...
                for (x <- 1 to vArray.size) {
                        if ((sArray(nmapping(x)-1)._2._2)-(vArray(x-1)._2._2) < 0) {
                                ...
                                sArray(nmapping(x)-1) = (nmapping(x).toLong, 
							(sArray(nmapping(x)-1)._2._1,
							(sArray(nmapping(x)-1)._2._2)-(vArray(x-1)._2._2)))
                        }
                }
        }
